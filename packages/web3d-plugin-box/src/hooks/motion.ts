...
const calculateMotion = (
  sortedElements: readonly TracedAElement[],
  ...
  const itemSpeed = sortedElements.map((el, index, arr) => {
    ...
    const timeDelta =
      frames[next.frameIndex].timestamp - frames[prev.frameIndex].timestamp;
  ...
  return itemSpeed.map(([uuid, speed], index, arr) => {
    ...
    const timeDelta =
      frames[sortedElements[nextId].frameIndex].timestamp -
      frames[sortedElements[prevId].frameIndex].timestamp;
    return [
      uuid,
      {
        speed,
