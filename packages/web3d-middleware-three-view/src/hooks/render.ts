...
export const useRender = (containers: Containers) => {
  ...
  const getCamera = (
    name: "front" | "side" | "top",
    rbox: MaybeRefOrGetter<RBox | undefined>
  ) => {
    const vbox = toValue(rbox);
    if (!vbox) return;
    const box = isChanging.value[name]
      ? vbox
      : {
          ...vbox,
    ...
    if (box && containerSize.width > 0 && containerSize.height > 0) {
      ...
      camera.up.set(
        ...views[name].UP.clone().applyQuaternion(rotation).toArray()
      );
      camera.position.set(
        ...center
          .clone()
  ...
  const zoomCameraHandler = (name: "front" | "side" | "top") => {
    return (event: WheelEvent) => {
      zooms.value[name] = Math.max(
        Math.min(zooms.value[name] - event.deltaY / 400, 3),
        0.25
      );
      dirty = true;
  ...
  const renderTo = (
    cameraRef: MaybeRefOrGetter<THREE.Camera | undefined>,
    container: MaybeRefOrGetter<HTMLDivElement | undefined>
  ) => {
