...
const pointsCulled = (
  ...
  const intensityAttr = frame.points!.geometry.getAttribute(
    "intensity",
  ) as Float32BufferAttribute;
  frame.intersect(frustum, (_, i) => {
    points.push(i);
    const intensity = Math.min(
      Math.max(Math.floor(intensityAttr.array[i]), 0),
      255,
    );
  ...
  const intensityThreshold =
    (ostuIntensityThreshold + triangleIntensityThreshold) / 2.0;
  const highIntensityPoints = points.filter(
    (i) => intensityAttr.array[i] >= intensityThreshold,
  );
...
const buildMergePoints = (allPoints: [ITFrame, number[]][]) => {
  ...
  allPoints.forEach(([frame, points]) => {
    const framePosition = frame.points!.geometry.getAttribute(
      "position",
    ) as Float32BufferAttribute;
...
export const useLineCompletion = (
  ...
  const lines = points
    .map((p, i, arr) => new Line3(p, arr[(i + 1) % arr.length]))
    .slice(0, -1);
  ...
  for (const key in tmp) {
    ...
    if (arr && arr.length > 2) {
      let weightSum = 0;
      const mean = arr.reduce(
        (acc, { distance, pos, point }) => {
          const weight = Math.min(
            Math.pow(1 / distance, inverseDistanceWeightingPow),
            10000,
          ); // 反距离权重, 限制最高10000
          weightSum += weight;
    ...
    while (s < result.length) {
      ...
      if (end - s > 3) {
        const newPoints = gaussianSmooth3d(
          result.slice(s, end).map(({ point }) => point),
          gaussianKernel,
        );
  ...
  const rearrangeResult = rearrange(
