...
describe("useLineCompletion", () => {
  it("should use cull points correctly", () => {
    ...
    const inputPoints = [new Vector3(0, 0, 0), new Vector3(5, 0, 0)];
    // Call useLineCompletion with mock data
    const mergedPoints = multiFramePointsCulled(frames, inputPoints, 0.5);

    expect(mergedPoints).toHaveProperty("position");
    expect(mergedPoints.position.array).toEqual(
  ...
  it("should use line completion correctly", () => {
    ...
    const inputPoints = [new Vector3(0, 0, 0), new Vector3(10, 0, 0)];

    const { result } = useLineCompletion(frames, inputPoints);
    const line = new Line3(new Vector3(0, 0, 0), new Vector3(50, 0, 0));
