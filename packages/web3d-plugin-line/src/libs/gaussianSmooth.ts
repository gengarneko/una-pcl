  ...
  for (let i = 0; i < array.length; i++) {
    ...
    for (let j = 0; j < kernel.length; j++) {
      ...
      const value =
        x >= 0 && x < array.length
          ? array[x]
          : array[Math.max(0, Math.min(array.length - 1, x))];
...
export const gaussianSmooth3d = (
  points: Vector3[],
  sigma: number,
): Vector3[] => {
  const filteredX = gaussianSmooth1d(
    points.map((p) => p.x),
    sigma,
  );
  const filteredY = gaussianSmooth1d(
    points.map((p) => p.y),
    sigma,
  );
  const filteredZ = gaussianSmooth1d(
    points.map((p) => p.z),
    sigma,
  );
  ...
  for (let i = 0; i < filteredX.length; i++) {
    filteredPositions.push(
      new Vector3(filteredX[i], filteredY[i], filteredZ[i]),
    );
  }
