...
const buildOctree = (points: Points) => {
  ...
  return localforage
    .getItem<SpatialTreeSerialization>(key)
    .then((data) => {
      if (data) {
    ...
    .then((octree) => {
      if (octree) {
        octree.intersect = measure(
          "web3d::octree::intersect",
          octree.intersect,
        );
      }
      return octree;
...
const buildQuadTree = (points: Points) => {
  ...
  return localforage
    ...
    .then((quadtree) => {
      if (quadtree) {
        quadtree.intersect = measure(
          "web3d::quadtree::intersect",
          quadtree.intersect,
        );
      }
      return quadtree;
          ...
          const lodIndices = [
            {
              distance: -1,
              k: 1,
              index: new THREE.BufferAttribute(tree.index, 1),
            },
          ];
          const lodConfig = [
            { distance: 50, k: 1.2 },
