...
export class TFrustumCulledPoints extends THREE.Points {
  ...
  constructor(
    points: THREE.Points,
    octree: Octree,
    indexLod: readonly LodIndexConfig[],
    depth: number = 0,
  ) {
    ...
    if (octree.subTrees.length > 0) {
      ...
      for (const subTree of octree.subTrees) {
        this.add(
          new TFrustumCulledPoints(points, subTree, indexLod, depth + 1),
        );
      }
    } else {
      this.geometry.setDrawRange(
        octree.index.byteOffset / 4,
        octree.index.byteLength / 4,
      );
    }
  }
    ...
    if (frustum.intersectsBox(this.octree.box)) {
      ...
      if (this.children.length > 0) {
        ...
        if (n < 1_000_000 && this.octree.inside(frustum)) {
          ...
          this.geometry.setDrawRange(
            Math.ceil(
              this.octree.index.byteOffset / 4 / this.indexLod[level].k,
            ),
            Math.floor(
              this.octree.index.byteLength / 4 / this.indexLod[level].k,
            ),
          );
