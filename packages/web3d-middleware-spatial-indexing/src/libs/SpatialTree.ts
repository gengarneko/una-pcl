import { Box3, BufferAttribute, BufferGeometry, Ray, Vector3 } from "three";
...
export abstract class SpatialTree {
  ...
  forEachPoint(callback: (p: Vector3, i: number) => void) {
    for (let i = 0; i < this.index.length; i++) {
      callback(
        _v1.fromBufferAttribute(this.position, this.index[i]),
        this.index[i],
      );
    }
  }
  ...
  intersect(
    ...
    callback: (point: Vector3, i: number) => void,
  ) {
  ...
  intersectRay(
    ...
    if (
      !obj.intersectsBox(
        _box.copy(this.box).expandByVector(new Vector3(d, d, d)),
      )
    ) {
      return;
    }
 ...
 */
export const fromSerialization = <T extends SpatialTree>(
  geometry: BufferGeometry,
  data: SpatialTreeSerialization,
  ...
  for (let i = 1; i < trees.length; i++) {
    const subtree = create(
      ...
      index.subarray(trees[i].offset, trees[i].offset + trees[i].length),
      position,
    );
 ...
 */
export const fromPointsGeometry = <T extends SpatialTree>(
  geometry: BufferGeometry,
  level: number,
