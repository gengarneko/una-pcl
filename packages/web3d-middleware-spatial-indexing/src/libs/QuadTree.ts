import { Box3, BufferAttribute, BufferGeometry, Points, Vector3 } from "three";
import {
  SpatialTree,
  SpatialTreeSerialization,
  fromPointsGeometry,
  fromSerialization,
  pointSort,
} from "./SpatialTree";
...
export class QuadTree extends SpatialTree {
  ...
  split(level: number, maxCount: number) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (l > 0) {
        ...
        const subTree = new QuadTree(
          subBox,
          this.index.subarray(split[i], split[i + 1]),
          this.position,
        );
  ...
  static fromSerialization(
    ...
    return fromSerialization(
      geometry,
      data,
      (box, index, position) => new QuadTree(box, index, position),
    );
  }
   ...
   */
  static fromPointsGeometry(
    geometry: BufferGeometry,
    level = 8,
    maxPointCount = 256,
  ): QuadTree {
    return fromPointsGeometry(
      geometry,
      level,
      maxPointCount,
      (box, index, position) => new QuadTree(box, index, position),
