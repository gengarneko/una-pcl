<template>
  <div class="flex flex-col">
    <div class="mb-2 text-sm font-bold">Segmentation Category</div>
    <div class="mb-2 text-sm">
      active category id:
      <span class="text-gray-200">{{ mainLabelID }}</span>
    </div>
    <table>
      <tbody>
        <tr
        ...
        >
          ...
          <td>
            {{
              activeFrames.reduce(
                (sum, frame) => sum + instance.counts[frame.index],
                0
              )
            }}
          </td>
...
const { show, hide, lock, unlock, setMainLabelID, changeColor } = useActions();
</script>
