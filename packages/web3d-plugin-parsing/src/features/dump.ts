...
const buildFields = (fields: readonly PCDFields[]): PCDFieldsWithOffset => {
  ...
  return {
    ...
    value: (geometry, pos, mat) =>
      fields.flatMap((f) => f.value(geometry, pos, mat)),
    offset: fields
      .flatMap((f) => f.size)
      .map((v) => {
        _offset += v;
...
export const dumpAscii = (
  frames: readonly TFrame[],
  fields: readonly PCDFields[],
) => {
  const mergeFields = buildFields(fields);
  const pointCount = frames.reduce(
    (acc, frame) => acc + frame.points!.geometry.attributes.position.count,
    0,
  );
  ...
  const buffer = frames
    .map((frame) => {
      ...
      return Array.from({ length: c })
        .map((_, i) =>
          mergeFields.value(geometry, i, frame.local.matrixWorld).join(" "),
        )
        .join("\n");
    })
    .join("\n");
...
export const dumpBinary = (
  ...
  const pointBufferSize = fields.reduce(
    (acc, field) => acc + field.size.reduce((acc, item) => acc + item, 0),
    0,
  );

  const headerBuffer = new TextEncoder().encode(header);
  const buffer = new ArrayBuffer(
    headerBuffer.byteLength + pointBufferSize * pointCount,
  );
  ...
  frames.forEach((frame) => {
    ...
    for (let i = 0; i < c; i++) {
      ...
      for (let j = 0; j < values.length; ++j) {
        ...
        const offset =
          frameOffset + mergeFields.offset[j] + i * pointBufferSize;
        dataViewHelper(
          bufferView,
          offset,
          value,
