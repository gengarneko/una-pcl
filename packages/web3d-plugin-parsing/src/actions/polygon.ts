...
import { Operation, TFrame, useDrama } from "@una-pcl/web3d";
...
export const polylineAction = (
  ...
  const tpoints = [...points, points[0]].map(
  ...
  activeFrames.value.forEach((frame) => {
    const result: number[] = [];
    const frameProjScreenMatrix = cameraProjScreenMatrix
      .clone()
