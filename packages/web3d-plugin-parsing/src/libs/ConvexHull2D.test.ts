...
describe("convexHull2D", () => {
  ...
  test("test bug", () => {
    const points: THREE.Vector2[] = [
      ...
      new THREE.Vector2(-0.27, -0.094959),
      new THREE.Vector2(-0.2625, 0.0879),
      new THREE.Vector2(-0.2625, 0.245),
      new THREE.Vector2(-0.3972, -0.0644),
    ];
    const expected: THREE.Vector2[] = [
      new THREE.Vector2(-0.475, -0.055),
      new THREE.Vector2(-0.27, -0.094959),
...
describe("point in polygon", () => {
  ...
  test("test point not in polygon (bugfix 2)", () => {
    ...
    const inCases: THREE.Vector2[] = [new THREE.Vector2(4.1, 3)];
    for (const p of inCases) {
      expect(pointInPolygon(points, p)).toBe(false);
    }
  });
