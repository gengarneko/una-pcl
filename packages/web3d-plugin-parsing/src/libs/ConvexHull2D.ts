...
export const convexHull2D = (p: THREE.Vector2[]): THREE.Vector2[] => {
  ...
  for (let i = 1; i < n; ++i) {
    while (stack.length > 1) {
      _v1
        .copy(points[stack[stack.length - 1]])
        .sub(points[stack[stack.length - 2]]);
...
const projectionToLine = (
  ...
  return a
    .clone()
    .add(
      line.clone().multiplyScalar(line.dot(p.clone().sub(a)) / line.dot(line)),
    );
};
...
export const rotatingCalipers = (points: THREE.Vector2[]) => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    while (
      line.cross(points[d].clone().sub(points[i])) <
      line.cross(points[(d % n) + 1].clone().sub(points[i]))
    ) {
      d = (d % n) + 1;
    }
    const lineN = new THREE.Vector2(line.y, -line.x);
    while (
      lineN.cross(points[c].clone().sub(points[i])) <
      lineN.cross(points[(c % n) + 1].clone().sub(points[i]))
    ) {
      c = (c % n) + 1;
    }
    if (i === 0) {
    ...
    while (
      lineNI.cross(points[e].clone().sub(points[i])) <
      lineNI.cross(points[(e % n) + 1].clone().sub(points[i]))
    ) {
      e = (e % n) + 1;
    }
  ...
  return {
    area: ans,
    rect: [result[0], result[1], result[3], result[2]],
    points: [points[pi], points[pi + 1], points[pc], points[pd], points[pe]],
  };
};
