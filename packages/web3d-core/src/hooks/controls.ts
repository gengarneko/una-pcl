...
export const useControls = (camera: THREE.Camera, renderer: THREE.Renderer) => {
  ...
  transform.addEventListener("dragging-changed", (event) => {
    ...
    if (
      ...
      confirmCallback &&
      transform.object &&
      attachedObj === transform.object
    ) {
      const box = {
        position: {
          x: transform.object.position.x,
          y: transform.object.position.y,
          z: transform.object.position.z,
        },
        rotation: {
          x: transform.object.rotation.x,
          y: transform.object.rotation.y,
          z: transform.object.rotation.z,
        },
        size: {
          x: transform.object.scale.x,
          y: transform.object.scale.y,
          z: transform.object.scale.z,
        },
      } as RBox;
      confirmCallback(box);
    }
