...
class CSS2DRenderer {
  ...
  render(scene: Scene, camera: Camera) {
    const frame = (
      parseInt(this.domElement.getAttribute("data-frame") || "0", 10) + 1
    ).toString();
  ...
  private _renderObjects(
    ...
    if ((object as any).isCSS2DObject) {
      ...
      if (visible) {
        ...
        if (
          !css2dObject.element.parentNode ||
          css2dObject.element.parentNode !== this.domElement
        ) {
      ...
      this._cache.objects.set(object, {
        distanceToCameraSquared: this._vector.distanceToSquared(
          camera.position,
        ),
      });
    }
