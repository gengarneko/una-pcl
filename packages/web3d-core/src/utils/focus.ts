...
export const useFocus = <
  A extends AElement,
  T extends THREE.Object3D<TFocusableEventMap>,
>(
  elements: Ref<A[]>,
  objs: Map<string, T>,
) => {
...
export const useSetFocusOnClick = <
  A extends AElement,
  T extends THREE.Object3D<TFocusableEventMap>,
>(
  focused: Ref<AElement | undefined>,
  objs: Map<string, T>,
  obj2element: (t: T) => A,
) => {
  ...
  const { off: stop } = onAdvanceMouseEvent((event) => {
    if (event.type === "click") {
      ...
      const items = Array.from(objs, (entry) => entry[1]).filter(
        (item) => item.visible && item.parent?.visible,
      );
