...
export const useAnswerStore = defineStore("answer", () => {
  ...
  const applyOperationEvent = createEventHook<{
    answer: AnswerContent;
    operation: Operation;
  }>();
  const composedApplyOperation = new Composer<{
    answer: AnswerContent;
    operation: Operation;
  }>();
  ...
  return {
    ...
    onSetupAnswer: setupAnswerEvent.on,
    applyOperation,
    useApplyOperation: composedApplyOperation.use.bind(composedApplyOperation),
    onApplyOperation: applyOperationEvent.on,
  } as const;
});
